...
public class AdventOfCode028 {
    ...
    private static int followSand(Set<Coordinates> rocks) {
        ...
        int floor = rocks.stream().mapToInt(Coordinates::x).max().orElse(Integer.MAX_VALUE) + 2;

        for (int i = 0; i < 1000; i++) {
            rocks.add(new Coordinates(floor, i));
        }

        Optional<Coordinates> sand = Optional.empty();
        while (!sand.equals(Optional.of(STARTING_POINT))) {
            sand = moveSandTillItStops(Optional.of(STARTING_POINT), rocks, sands);
